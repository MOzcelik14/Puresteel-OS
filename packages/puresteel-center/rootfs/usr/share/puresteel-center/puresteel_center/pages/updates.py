...
class UpdatesPage(QWidget):
    ...
    def show(self,a,f,err):
        puresteel=[item for item in a if item[0].startswith("puresteel-")]
        critical,normal=classify_apt([item for item in a if not item[0].startswith("puresteel-")])
        total=len(a)+len(f)
        tr=self.lang=="tr"
        self.add_group("Puresteel Rolling • imzalı APT" if tr else "Puresteel Rolling • signed APT",puresteel)
